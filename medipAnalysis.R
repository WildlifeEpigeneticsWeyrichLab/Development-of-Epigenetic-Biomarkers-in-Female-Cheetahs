...
processedBamFiles <- lapply(X = paste(dataDirectory, bamFileName, sep = ""), 
...
mset2<-processedBamFiles[comparison[[analysis]]$mset2[c(1,2,4,5,6,7,8)]]
...
for (analysis in 1:length(comparison)) {  
  ...
  cn <- c(seqFiles$sampleName[comparison[[analysis]]$mset1], seqFiles$sampleName[comparison[[analysis]]$mset2[c(1,2,4,5,6,7,8)]])
  ...
  treatgroup <- c(rep("mset1", length(mset1)), rep("mset2", 7))
  batch_effect <- c("batch1","batch1","batch1","batch1","batch1","batch2","batch2","batch2","batch1","batch1","batch1","batch1","batch2","batch2","batch2")
#  sex <- c(rep("f", 10),"m", rep("f", 5))
  ...
  design <- model.matrix(~batch_effect +  treatgroup)
 ...
 # lrt = exactTest(y) 
  ...
  methResults$MSets2.counts.mean <- rowMeans(methResults[match(paste(seqFiles$sampleName[comparison[[analysis]]$mset2[c(1,2,4,5,6,7,8)]], ".bam.counts", sep=""), colnames(methResults))])
  ...
  methResults$MSets2.rpkm.mean <- rowMeans(methResults[match(paste(seqFiles$sampleName[comparison[[analysis]]$mset2[c(1,2,4,5,6,7,8)]], ".bam.rpkm", sep=""), colnames(methResults))])
